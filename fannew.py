...
class Ui_MainWindow(object):
    ...
    def menu(self,action):
        ...
        if txt=='EVALUATE Team':
            from dlgscorenew import Ui_Dialog
    ...
    def criteria(self,ctgr,item):
        ...
        if ctgr=="BOWL" and self.bwl>=5:msg="bowlers not more than 5"
        if ctgr=="ALL" and self.ar>=3:msg="Allrounders not more than 3"
        ...
        if ctgr=="BOWL":self.bwl+=1
        if ctgr=="ALL":self.ar+=1
        if ctgr=="WK":self.wk+=1

        
        sql="SELECT Player_Value from Teams where Player_Name='"+item.text()+"'"
    ...
    def removelist1(self,item):
        ...
        if self.rbtn2.isChecked()==True:ctgr='BOWL'
        if self.rbtn3.isChecked()==True:ctgr='ALL'
    ...
    def removelist2(self,item):
        ...
        cursor = conn.execute("SELECT t2.Player_Name,t.Player_Value,t2.Category from Teams t,T20_Stats t2 where t.Player_Name=t2.Player_Name and t.Player_Name='"+item.text()+"'")
        row=cursor.fetchone()
    ...
    def fillList(self,ctgr):
        ...
        self.list1.clear()
        sql="SELECT Player_Name from T20_Stats where Category='"+ctgr+"';"
        cur=conn.execute(sql)
        for row in cur:
            selected=[]
    ...
    def openteam(self):
        ...
        sql="select Team_Name from Selected_Teams;"
        #print("efef")
        cur=conn.execute(sql)
        #print("vebjb")
        teams=[]
        ...
        sql1="SELECT Player_Name,Player_Value from Selected_Teams where Team_Name='"+team+"';"
        ...
        for i in range(count-1):
            ...
            #print(ply)
            sql="select Category from T20_Stats where Player_Name='"+ply+"';"
    ...
    def saveteam(self,nm,ply,val):
        ...
        if self.bat+self.bwl+self.ar+self.wk>11:
            self.showdlg("Only 11 Players allowed")
            return
        if self.bat+self.bwl+self.ar+self.wk<11:
        ...
        #print("frbfj")
        sql="INSERT INTO Selected_Teams (Team_Name,Player_Name,Player_Value) VALUES ('"+nm+"','"+ply+"','"+str(val)+"');"
        #print("f3f4")
        try:
            #print("bjr")
            cur=conn.execute(sql)
